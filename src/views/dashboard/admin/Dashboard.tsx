...
export default function Dashboard() {
  ...
  const menungguKonfirmasi = filteredPesanan.filter(p => p.status === "Menunggu Konfirmasi").length;
  const dalamProses = filteredPesanan.filter(p => p.status === "Diproses").length;
  ...
  const getMonthlyData = () => {
    ...
    while (currentDate <= endDate) {
      const month = currentDate.toLocaleString('id-ID', { month: 'short', year: 'numeric' });
            ...
            <>
              ...
              <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white p-6 shadow rounded-lg">
                  <h2 className="text-black-500 text-lg font-semibold mb-4">Trend Pesanan</h2>
                  <Line data={chartData} options={chartOptions} />
                </div>

                <div className="bg-white p-6 shadow rounded-lg">
                  <h2 className="text-black-500 text-lg font-semibold mb-4">Transaksi Terbaru</h2>
                  {filteredPesanan.length > 0 ? (
                    filteredPesanan.slice(0, 5).map((item, index) => (
                      <div key={index} className="mt-4 p-4 bg-gray-50 rounded-lg flex justify-between hover:bg-gray-100 transition-colors cursor-pointer">
                        <div>
                          <p className="font-semibold">{item.user?.name || 'Unknown'}</p>
                          <p className="text-gray-500 text-sm">{item.user?.phone || '-'}</p>
                        </div>
                        <p className="text-gray-500 text-sm max-w-[200px] truncate">{item.alamat}</p>
                        <div className="pr-3 rounded-lg flex flex-col items-end">
                          <p className="text-gray-600">{new Date(item.tanggal_pesanan).toLocaleDateString('id-ID')}</p>
                          <p className="text-gray-600">Rp. {item.total_harga?.toLocaleString()}</p>
                        </div>
                      </div>
                    ))
                  ) : (
                    <div className="text-center text-gray-500 py-8">
                      <Icon icon="mdi:inbox-outline" className="mx-auto mb-2" width={48} />
